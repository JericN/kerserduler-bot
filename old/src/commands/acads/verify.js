...
module.exports = {
    ...
    callback: async (client, interaction) => {
        ...
        const missingChannels = checkMissingChannels(Object.keys(validEvents), interaction.guild.channels);

        // get guild roles and check for missing roles
        const missingRoles = checkMissingRoles(Object.keys(validEvents), interaction.guild.roles);

        console.log(missingChannels, missingRoles, invalidEvents, validEvents);
...
function checkMissingChannels(subjects, channels) {
    return subjects.filter(
        subject => !channels.cache.some(ch => ch.type === 0 && ch.name.replace(/[\s_-]/g, '') == subject),
    );
}
